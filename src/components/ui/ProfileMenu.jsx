...
import { saveUserProfile, subscribeUserProfile } from '../../services/db'
...
export default function ProfileMenu({ onSignOut }) {
  ...
  // ── Load profile from Firestore (realtime) ──
  useEffect(() => {
    if (!user?.uid) return
    const unsub = subscribeUserProfile(user.uid, (profile) => {
      if (profile) {
        if (profile.name)     setName(profile.name)
        if (profile.desc)     setDesc(profile.desc)
  ...
  async function saveDesc() {
    setSaving(true)
    await saveUserProfile(user.uid, { desc })
    setSaving(false)
    setEditingDesc(false)
  }

  // ── Upload photo → save to Firestore ────────
  ...
  return (
    <div ref={menuRef} className={styles.wrap}>
      ...
      <button className={styles.avatarBtn} onClick={() => setOpen(o => !o)}>
        {displayPhoto ? (
          <img src={displayPhoto} alt="avatar" className={styles.avatarImg} />
        ) : (
          ...
          >
            ...
            <div className={styles.header}>
              <div className={styles.avatarLarge} onClick={() => fileRef.current?.click()}>
                {displayPhoto ? (
                  <img src={displayPhoto} alt="avatar" className={styles.avatarLargeImg} />
                ) : (
